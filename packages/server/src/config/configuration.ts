...
export class Configuration {
  ...
  public mcp_settings: Record<string, any> | null = null;
  ...
  static fromRunnableConfig(config?: RunnableConfig): Configuration {
    ...
    const fieldMappings = [
      { camelCase: 'resources', snakeCase: 'resources', envVar: 'RESOURCES' },
      { camelCase: 'max_plan_iterations', snakeCase: 'max_plan_iterations', envVar: 'MAX_PLAN_ITERATIONS' },
      { camelCase: 'max_step_num', snakeCase: 'max_step_num', envVar: 'MAX_STEP_NUM' },
      { camelCase: 'max_search_results', snakeCase: 'max_search_results', envVar: 'MAX_SEARCH_RESULTS' },
      { camelCase: 'mcp_settings', snakeCase: 'mcp_settings', envVar: 'MCP_SETTINGS' }
    ];
